...
public class ChannelRepository : IChannelRepository
{
    ...
    {
        List<Programme> list = new List<Programme>();
        List<XElement> xeList = xdElCinema.Root.Descendants("programme").Where(prg => prg.Attribute("title").Value.Contains(query) || prg.Element("desc").Value.Contains(query)).ToList();
